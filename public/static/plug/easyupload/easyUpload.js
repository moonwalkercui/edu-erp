; (function ($) {
  $.fn.easyUpload = function (opts) {
    var defaults = {
      allowFileTypes: '*.pdf;*.doc;*.docx;*.jpg,*.jpeg,*.gif,*.png,*.bmp',//允许上传文件类型，格式'*.pdf;*.doc;'
    ...
    this.each(function (index, element) {
      ...
      var easyManager = {
        init: function () {
          ...
          $html += '<span class="easy_upload_head_btn1 noselect">全部上传</span>';
        ...
        bindHead: function () {
          ...
          $('.fileInput').off('change').change(function () {
            ...
            // $(this).parent().find('.head_check').html('&#xe61e;').attr('data-checked', 'yes'); // 多选框
          });
        ...
        _renderFile: function (fileArr, target) {
          ...
          function render(file) {
            ...
            var sHtml = '<span class="queue_check" data-up="1" ></span>';
            ...
            $html += file.allow ? sHtml : '<span class="queue_check" data-up="6" ></span><p class="status status6">文件不允许</p>';
            $html += '</div>';
          ...
          for (var i = 0; i < fileArr.length; i++) {
            if (option.multi && option.multiNum !== 0) {
        ...
        _findItems: function(type,target) {
          var arr = [], items = $('.easy_upload_queue_item');
          for (var i = 0; i < items.length; i++) {
            if($(items[i]).find('.queue_check').attr('data-up') == '1')
              arr.push($(items[i]).attr('data-index'));
          }
          return arr;
        },
