...
<template>
	<view class="date">
		<view class="head">
			...
			<view class="right" @click="switch_today()">TODAY</view>
		</view>
		<view class="date_dl" >
...
export default {
	...
	watch:{
		value(val){
			this.get_date(this.date_parse(val));
		},
		dot_lists:{
			immediate:true,
			handler(val){
	...
	methods:{
		switch_today() {
			this.get_date((new Date()).getFullYear() + '/' + ((new Date()).getMonth() + 1) + '/' + (new Date()).getDate(), 'same');
			if(this.retract){
...
.head{
	...
	.right{
		position: absolute; right: 30rpx;background-color: #fff; padding: 8rpx 10rpx; border-radius: 6rpx;
