			...
			<scroll-view scroll-y="true">
				...
				<u-radio-group v-model="value" :wrap="true" @change="onChange" v-if="limit ==1">
					<u-radio v-for="(item,index) in list" :key="index" :name="item.id" style="padding: 10px"
						class="u-border-bottom">{{item.name}} <text style="font-size: 90%">￥{{item.price}}</text></u-radio>
				</u-radio-group>
				<u-checkbox-group :wrap="true" @change="onChange" v-else>
					<u-checkbox v-for="(item,index) in list" :key="index" :name="item.id" style="padding: 10px" v-model="item.checked" 
						class="u-border-bottom">{{item.name}} <text style="font-size: 90%">￥{{item.price}}</text></u-checkbox>
				</u-checkbox-group>
...
<script>
	...
	export default {
		...
		methods: {
			getList() {
				this.$http.get('tCenter/course/list', this.pageData, res => {
