<template>
	<view>
		<uni-nav-bar left-icon="arrow-left" right-icon="reload" title="课程列表" @clickLeft="clickLeft" @clickRight="clickRight"/>
		<view class="u-demo-area">
			<u-cell-group title="">
				<u-cell-item v-for="(item,index) in list" :key="index" center :is-link="true" index="index" @click="showDetail(item.id)" 
				:title-style="{'width': '75%'}" :arrow="true">
					<view slot="title">{{item.name}}</view>
					<view slot="label">{{item.lessonCount}}次 有效期{{item.expireMonths}}个月</view>
					<view slot="label">{{item.info}} </view>
...
<script>
	...
	export default {
		...
		methods: {
			handleReq() {
				this.$http.get('tCenter/course/list',{page: this.pageData.page}, res => {
