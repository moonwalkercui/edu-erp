<template>
	<view>
		<uni-nav-bar left-icon="arrow-left" right-icon="reload" title="报单记录" @clickLeft="clickLeft" @clickRight="clickRight"/>
		<view class="u-demo-area">
			<u-cell-group title="">
				<u-cell-item v-for="(item,index) in list" :key="index" center :is-link="true" index="index" @click="showDetail(item.id)" :arrow="true">
					...
					<view slot="label">学员: {{item.studentName}}</view>
					<view slot="label">{{item.startDate}} ~ {{item.expireDate}}</view>
					<view>￥{{item.amount}}</view>
				</u-cell-item>
			</u-cell-group>
			
			<view class="u-padding-30 bg-white" v-if="list.length == 0" >
				<u-empty mode="list" text="暂无报单信息"></u-empty>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			handleReq() {
				this.$http.get('tCenter/course/contractRecord',{page: this.pageData.page}, res => {
			...
			showDetail(id) {
				uni.navigateTo({
					url: `/pages/contract/detail?contractId=${id}`
				});
			},
			clickRight() {
				this.list = [];
