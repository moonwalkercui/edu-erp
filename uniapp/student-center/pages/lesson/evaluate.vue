<template>
	<view>
		<uni-nav-bar left-icon="arrow-left" right-icon="reload" title="上课点评" @clickLeft="clickLeft" @clickRight="clickRight"/>
		<view class="u-demo-area">
			<u-cell-group title="">
				<u-cell-item v-for="(item,index) in list" :key="index" center :is-link="false" index="index"
				:value-style="{'width': '350rpx'}" :arrow="false">
					<view slot="title">{{item.evaluation}}</view>
...
<script>
	...
	export default {
		...
		methods: {
			handleReq() {
				this.$http.get('sCenter/lesson/evaluateLog',{page: this.pageData.page}, res => {
					if(!res) return;
